...
@celery_app.task(name='get match replay', retries=3, default_retry_delay=7)
def get_match_replay(match_id: int, parser_first: bool = True):
    ...
    if not is_valid:
        parse_replay(dem_file, jsonl_file, parser_port)
