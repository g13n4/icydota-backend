...
from .columns_to_postprocess import (
    AVERAGE_TOTAL_DATA,
    GAME_COLUMNS,
    LANE_COLUMNS,
    MAX_TOTAL_DATA,
    ...
    return [
        {
            "slot": slot_k,
            "windows_data": ck,
            **{k: v for k, v in cv.items() if k not in db_names},
        }
        for slot_k, slot_v in data.items()
...
def _get_df_slice(
    ...
    df_slice.set_index("windows_data", inplace=True)
...
def compare_position_performance(
    ...
    for player in MPD.get_all():
        ...
        get_df_slice = partial(
            _get_df_slice, df=data_df
        )  # columns are turned into index
...
def fill_total_values(data_df: pd.DataFrame) -> pd.DataFrame:
    data_df.set_index(["windows_data", "slot"], inplace=True)

    data_df.loc[(SUM_TOTAL_DATA, "ltotal")] = data_df.loc[
        (SUM_TOTAL_DATA, LANE_COLUMNS)
    ].sum(axis=1)

    data_df.loc[(SUM_TOTAL_DATA, "gtotal")] = data_df.loc[
        (SUM_TOTAL_DATA, GAME_COLUMNS)
    ].sum(axis=1)

    data_df.loc[(MAX_TOTAL_DATA, "ltotal")] = data_df.loc[
        (MAX_TOTAL_DATA, LANE_COLUMNS)
    ].max(axis=1)

    data_df.loc[(MAX_TOTAL_DATA, "gtotal")] = data_df.loc[
        (MAX_TOTAL_DATA, GAME_COLUMNS)
    ].max(axis=1)

    data_df.loc[(AVERAGE_TOTAL_DATA, "ltotal")] = data_df.loc[
        (AVERAGE_TOTAL_DATA, LANE_COLUMNS)
    ].mean(axis=1)

    data_df.loc[(AVERAGE_TOTAL_DATA, "gtotal")] = data_df.loc[
        (AVERAGE_TOTAL_DATA, GAME_COLUMNS)
    ].mean(axis=1)
...
def postprocess_data(
    ...
    data_df = pd.DataFrame(
        _flatten_for_pd(data, ["_parsing_name", "_db_name"]),
    )
    ...
    [
        data_df[col].astype(np.float64)
        for col in data_df.columns
        if col in MS.window_values_names
    ]
