...
def _parse_replay_v2(match_id: int, folder_path: Path, overwrite: bool = False) -> None:
    ...
    curl_file_path = f'"@{str(BASE_PATH)}/{match_id}/{match_id}.dem"'  # double quotes are intentional
    curl_file_output = f'"{str(BASE_PATH)}/{match_id}/{match_id}.jsonl"'
...
def get_match_replay(match_id: int, overwrite: bool = False) -> None:
