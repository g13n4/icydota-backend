...
from models import PerformanceDataType, PerformanceDataDescription, PerformanceTotalData, ComparisonType, \
    PerformanceWindowData, PerformanceDataCategory, GamePerformance
...
def _fill_comparison_pws(db_session,
    ...
    for item in comparison_data:
        for df, percent_comparison in [(item['df_flat'], False), (item['df_percent'], False)]:
            for index, line in iterate_df(df, use_offset=False):
                ...
                ctp_obj = compare_performance(comparandum_total_obj,
                                              comparans_total_obj,
                                              PerformanceTotalData,
                                              percent_comparison)
                ctp_obj.data_info = PDD_T_obj
...
def process_main_replay_data(db_session,
    ...
    PTD_dict = _get_PDT_objects(db_session, PDC_objs, column_to_category_obj)
